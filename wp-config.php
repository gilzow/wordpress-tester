...
/**
 * Multisite support
 * if the MULTISITE env var has been set and true
 * AND either MULTISITEINSTALLED has been set and true OR the file located in PSHMULTISITEFILE exists
 * then we can continue with our m
 */
if(
	filter_var(getenv('MULTISITE'),FILTER_VALIDATE_BOOL)
	&& ( filter_var(getenv('MULTISITEINSTALLED'),FILTER_VALIDATE_BOOL)
		|| (
			false !== getenv('PSHMULTISITEFILE') && is_file(getenv('PSHMULTISITEFILE'))
		)
	)
) {
